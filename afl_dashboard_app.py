...
import requests
import streamlit.components.v1 as components  # for sidebar form
...
def highlight_positive_edge(row):
    ...
    if pd.notnull(edge):
        if edge > 0:
            return ["background-color: #e9f9ec"] * len(row)
        else:
            return ["background-color: #faeaea"] * len(row)
    else:
        return [""] * len(row)
...
# ----------------------------------------------------
    ...
    try:
        ...
        st.error(f"Error loading ExportDisposals.xlsx: {e}")
